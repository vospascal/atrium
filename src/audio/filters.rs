//! Shared IIR filter primitives.
//!
//! All coefficient formulas from RBJ Audio EQ Cookbook.

use super::atmosphere::{air_absorption_shelf_gains, AtmosphericParams};

// ─────────────────────────────────────────────────────────────────────────────
// Biquad — 2nd-order IIR filter (Direct Form I)
// ─────────────────────────────────────────────────────────────────────────────

/// 2nd-order IIR biquad filter (Direct Form I).
///
/// Supports lowpass, highpass, low-shelf, and high-shelf configurations.
/// Coefficient update methods (`set_*`) preserve filter state to avoid clicks.
/// Constructor methods (`lowpass`, `highpass`) start with zeroed state.
#[derive(Clone)]
pub struct Biquad {
    b0: f32,
    b1: f32,
...
impl Biquad {
    ...
    /// Create a Butterworth lowpass filter (Q = 1/√2).
    ...
    /// Create a Butterworth highpass filter (Q = 1/√2).
    ...
    pub fn set_low_shelf(&mut self, freq_hz: f32, gain_db: f32, sample_rate: f32) {
        if gain_db.abs() < 0.01 {
            self.set_unity();
            return;
        }
    ...
    /// Preserves filter state to avoid clicks on parameter changes.
    pub fn set_high_shelf(&mut self, freq_hz: f32, gain_db: f32, sample_rate: f32) {
        if gain_db.abs() < 0.01 {
            self.set_unity();
            return;
        }
    ...
    /// Set peaking/notch EQ coefficients (RBJ Audio EQ Cookbook).
    /// Preserves filter state to avoid clicks on parameter changes.
    pub fn set_peak(&mut self, freq_hz: f32, gain_db: f32, q: f32, sample_rate: f32) {
        if gain_db.abs() < 0.01 {
            self.set_unity();
            return;
        }
    ...
    /// Reset coefficients to unity passthrough (preserves filter state).
...
// ─────────────────────────────────────────────────────────────────────────────
// AirAbsorptionFilter — 2-band shelving for ISO 9613-1
// ─────────────────────────────────────────────────────────────────────────────
...
/// 0.5 dB hysteresis to avoid unnecessary coefficient recomputation.
