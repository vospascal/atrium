...
//! performance) live in the `voxel-game-ui` crate; nothing here
//! is reachable from them, which is what makes the two separable into their own crates.
...
pub fn graph_drawer_height(ui: &egui::Ui, state: &GraphEditorState) -> f32 {
    if state.visible {
        ...
        if let Some(layer) = registry.declarations().find(|declaration| {
            declaration.kinds.contains(&state.graph.kind)
                && declaration.operation
                    == voxel_rt::graph::NodeOperation::Material(
