...
//! | `emission` | CAGI emissive injection (E5) | all-zero, see below |
...
//!
//! **Every `emission` row is zero**, because no emissive voxel type exists in
//! the 24-variant `Voxel` enum — there is nothing to light with yet. Adding
//! `Lantern` / `GlowBerry` reaches into `voxel-core` and worldgen, so it is
//! M1b, not M1.
//!
//! ## The acoustic column
//!
...
/// this a few times per frame). `liquid_predicate_agrees_with_the_table` pins the
/// two together.
...
pub struct Material {
    ...
    pub albedo: [f32; 3],
    /// Emitted radiance, linear. All-zero until M1b adds emissive voxel types.
    pub emission: [f32; 3],
...
/// The uploaded row: std430-clean at 48 bytes (three 16-byte rows, each a
...
pub fn materials() -> Vec<Material> {
    ...
    vec![
        ...
        // 7  Water — the only row carrying real work: smooth (specular
        // highlights and, later, reflections), see-through, and transporting
        // most of the light that enters it.
        Material {
            name: "water",
...
mod tests {
    ...
    /// The GPU row must stay 48 bytes with no interior padding, or the WGSL
    /// `array<Material>` stride silently disagrees with the upload.
    #[test]
    fn gpu_row_layout_matches_wgsl() {
        assert_eq!(std::mem::size_of::<GpuMaterial>(), 48);
        assert_eq!(std::mem::align_of::<GpuMaterial>(), 4);
        assert_eq!(MATERIAL_TABLE_BYTES, 1152);
    }
